...
_page__WEBPACK_IMPORTED_MODULE_3__.default.appendElements();
...
var fetchError = function fetchError(err) {
  mainEl.classList.add('none');
  perror.innerHTML = "".concat(err.message, "... <br>We are so sorry, Your city cannot be located");
};

var removeNone = function removeNone(ele) {
...
var checkResult = function checkResult(res) {
  if (res.cod === '404') fetchError(res);else removeNone(mainEl);
};

var conversion = function conversion(tem) {
  var faren = tem * 1.8 + 32;
  if (header.textContent.includes('°C')) header.textContent = "".concat(faren, "\xB0F");else header.textContent = "".concat(tem, "\xB0C");
  return header;
};

var country = function country(cty) {
  return country_code_lookup__WEBPACK_IMPORTED_MODULE_1__.byIso(cty);
};

var mins = function mins(time) {
  var timns = time / 60;
  var curr = moment__WEBPACK_IMPORTED_MODULE_2___default()().utcOffset(timns).format('MMMM DD, YYYY-h:mm A');
  return curr.split('-');
};

submit.addEventListener('click', /*#__PURE__*/function () {
  var _ref2 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2(e) {
    var query, apiKey, url, result, main, name, sys, timezone, weather, dateTime, content;
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            e.preventDefault();
            query = search.value;
            apiKey = '1167e47fcac43a43c26e928ef53a787b';
            url = "https://api.openweathermap.org/data/2.5/weather?q=".concat(query, "&units=metric&appid=").concat(apiKey);
            _context2.next = 6;
            return getData(url)["catch"](function (err) {
              header.innerHTML = err;
            });

          case 6:
            result = _context2.sent;
            main = result.main, name = result.name, sys = result.sys, timezone = result.timezone, weather = result.weather;
            form.reset();
            checkResult(result);
            btn.classList.remove('none');
            dateTime = mins(timezone);
            date.textContent = dateTime.shift();
            time.textContent = dateTime.pop();
            code = country(sys.country);
            content = weather[0];
            header.textContent = "".concat(main.temp, "\xB0C");
            para.innerHTML = "".concat(name, ", ").concat(code.country);
            figcaption.textContent = content.main;
            weatherTemp = main.temp;
            desc.textContent = content.description;
            img.src = "https://openweathermap.org/img/wn/".concat(content.icon, "@2x.png");
...
btn.addEventListener('click', /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee3() {
  var num;
  return regeneratorRuntime.wrap(function _callee3$(_context3) {
    while (1) {
      switch (_context3.prev = _context3.next) {
        case 0:
          num = Number(weatherTemp);
          conversion(num);
...
var page = function () {
  var createList = function createList(arr, pointer, par) {
    var fil = arr.filter(function (obj) {
      return obj.hasOwnProperty(pointer);
    }); //eslint-disable-line

    var cur = fil[0];
    var elem;
    cur[pointer].forEach(function (el) {
      if (pointer === 'fg') elem = document.createElement(el);else elem = document.createElement(pointer);
  ...
  var useRes = function useRes(el, pointer, info) {
    var _info = _slicedToArray(info, 1),
        key = _info[0];

    var keys = key[pointer];
    Object.keys(keys).forEach(function (kee) {
      el.setAttribute(kee, keys[kee]);
    });
  };

  var setAttributes = function setAttributes(el, pointer, arr) {
  ...
  var appendElements = function appendElements() {
    var figs = createList(list, 'fg', figure);
    var uls = createList(list, 'li', ul);
    var fom = setAttributes(form, 'form', ele);
    var sach = setAttributes(input, 'search', ele);
    var butn = setAttributes(btnInput, 'btn', ele);
  ...
  var setContent = function setContent(el, val) {
    el.textContent = val;
  };
...
/*!***************************************************!*\
  ...
  \***************************************************/
/*! default exports */
/*! export byCountry [provided] [no usage info] [missing usage info prevents renaming] */
/*! export byFips [provided] [no usage info] [missing usage info prevents renaming] */
/*! export byInternet [provided] [no usage info] [missing usage info prevents renaming] */
/*! export byIso [provided] [no usage info] [missing usage info prevents renaming] */
...
/* harmony import */ var _node_modules_css_loader_dist_cjs_js_main_css__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! !!../../../node_modules/css-loader/dist/cjs.js!./main.css */ "./node_modules/css-loader/dist/cjs.js!./src/assets/styles/main.css");
